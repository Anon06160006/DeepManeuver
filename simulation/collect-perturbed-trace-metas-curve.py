...
def add_qr_cubes(scenario):
    global qr_positions
    qr_positions = []
    with open('posefiles/qr_box_locations-curve4.txt', 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            line = line.split(' ')
            pos = line[0].split(',')
...
def main():
    ...
    steps_per_sec = 15
    samples = 25
    newdir = f"EXPERIMENT1-{default_spawnpoint}-max{direction}-new{model._get_name()}model-{steps_per_sec}Hz-{''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6))}"
                    ...
                    for nl in noiselevels:
                        for cuton in cutons:
                            for i in range(samples):
                                ...
                                # training_file = "{}/{}-{}-{}-{}-bbsize{}-{}iters-noisedenom{}-distcuton{}-rscutoff{}.pickle".format(new_results_dir, technique,
                                #                                                                                      model._get_name(), direction,
                                ...
                                if technique == "dbb":
                                    ...
                                    results = run_scenario_superdeepbillboard(vehicle, bng, model, spawn, direction, collect_sequence_results=unperturbed_results,
                                                                              ...
                                                                              dist_to_bb_cutoff=rsc, resultsdir=new_results_dir, input_divers=True, loss_fxn=lossname)
